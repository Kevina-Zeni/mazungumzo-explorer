                ...
                if st.button("Identify speakers"):
                    with st.spinner("Identifying speakers (slower step, runs on CPU)..."):
                        ...
                        try:
                            ...
                            st.error(f"Diarization failed: {type(e).__name__}: {e}")
                            st.info("This step is memory-intensive and may fail on limited-resource hosting. "
                                     "Try a shorter clip, or check the app's resource logs on Streamlit Cloud's manage page.")
                            st.stop()
