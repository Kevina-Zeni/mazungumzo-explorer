...
with tab_audio:
    ...
    st.caption("Upload a clip under 90 seconds — keeps processing fast and free "
               "(no hosted GPU or paid diarization API).")
    ...
    if audio_file is not None:
        ...
        if duration is not None and duration > MAX_CLIP_SECONDS:
            ...
            # STEP 3 — Diarization (local CPU pyannote via whisperx wrapper)
            # ---------------------------------------------------------------
            if "demo_transcript_segments" in st.session_state:
                if st.button("Identify speakers"):
                    with st.spinner("Identifying speakers (slower step, runs on CPU)..."):
                        ...
                        def load_diarization_pipeline():
                            return DiarizationPipeline(token=st.secrets["HF_TOKEN"], device="cpu")

                        diarize_model = load_diarization_pipeline()
                        diarize_df = diarize_model(tmp_path)
                        ...
                        st.success(f"Found {diarize_df['speaker'].nunique()} speaker(s).")

           
            # STEP 3b — Merge transcript segments with speaker segments
            # ---------------------------------------------------------------
            if "demo_speaker_segments" in st.session_state and "demo_transcript_segments" in st.session_state:
                ...
                # ---------------------------------------------------------------
                # STEP 4 — Self-intro name autoscan (generic version — no known
                # roster exists for arbitrary uploads, unlike the corpus's
                # detect_self_introductions which matches against correct_terms).
                # Falls back to generic Speaker labels where no intro is found.
                # ---------------------------------------------------------------
                ...
                # STEP 5 — Timeline chart + transcript display
                # ---------------------------------------------------------------
                ...
                # STEP 6 — Theme extraction + knowledge graph
                # 
                # ---------------------------------------------------------------
...
"""
                ...
                # STEP 7 —  RAG chat (in-memory, discarded per session)
                # ---------------------------------------------------------------
